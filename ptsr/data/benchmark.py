...
class Benchmark(srdata.SRData):
    ...
    def _set_filesystem(self, dir_data):
        ...
        print("{}: scale = {}".format(self.name, self.scale))
        # self.dir_hr = os.path.join(
        #     self.apath, "HR/{}/x{}".format(self.name, self.scale[0]))
        self.dir_hr = os.path.join(
            self.apath, "{}/HR".format(self.name))

        # self.dir_lr = os.path.join(self.apath,  "LRBI/{}/".format(self.name))
        self.dir_lr = os.path.join(self.apath, "{}/LR_bicubic/X{}".format(self.name, self.scale[0]))
        self.ext = ('.png', '.png')
        ...
        for f in names_hr:
            ...
            for si, s in enumerate(self.scale):
                names_lr[si].append(os.path.join(
                    # self.dir_lr, 'x{}/{}_LRBI_x{}{}'.format(s, filename, s, self.ext[1])
                    self.dir_lr, '{}x{}{}'.format(filename, s, self.ext[1])
                ))
